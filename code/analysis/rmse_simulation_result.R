rm(list=ls())
setwd("data")
simulation_setting = read.csv("./simulation/rmse/simulation_setting.csv")
...
rmse_result$varying_variable = rep(c("n_students", "icc",
                                     "tp", "tn"), each=15)
rmse_result$varying_values = c(seq(from=30, to=100, by=5),
                               seq(from=0, to=0.06, length.out=15),
                               seq(from=0.85, to=0.999, length.out=15),
                               seq(from=0.5, to=0.95, length.out=15))
...
for(i_setting in 1:nrow(simulation_setting))
{
  ...
  for(method in c("gmm", "reg", "mle"))
  {
    ...
    {
      est = readRDS(paste0("./simulation/rmse/setting_",
...
write.csv(rmse_result, file="./results/simulation/rmse/rmse_result.csv",
          row.names = FALSE)
